        ...
        switch (status) {
            ...
            case 'unsubscribed':
                request.put({
                    ...
                    // logger.debug('JSON.stringify(response)=' + JSON.stringify(response));
                    if (err) {
                        logger.error('Mailchimp update err, email=' + newUser.email + ', ' + err);
                        callback(null, 'fail', status);
                    }
                    else if (response.statusCode != 200) {
                        logger.error('Mailchimp update error status, email=' + newUser.email + ', statusCode=' + response.statusCode + ', title=' + response.body.title + ', ' + response.body.detail);
                        callback(null, 'fail', status);
                    }
                    else {
        ...
        User.create(newUser, function(err) {
            if (err) {
                if (err.message.indexOf("E11000") >= 0) { // duplicate username (can happen with counselor supporting more than one school); need to make up a unique username and try again
                    ...
                    logger.error("Non-dup system error on user create.");
